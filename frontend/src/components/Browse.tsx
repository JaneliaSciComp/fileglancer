...
export default function Browse() {
  ...
  return (
    <div
      className="flex flex-col h-full max-h-full min-w-fit max-w-full"
      data-browse-container
      onPaste={async event => {
    ...
    >
      ...
      />
      <div
        className={`relative grow min-w-fit shrink-0 max-h-[calc(100%-55px)] flex flex-col overflow-y-auto px-2 ${!fspName ? 'bg-surface-light py-6' : ''}`}
      >
        {!fspName ? (
