...
export default function CreateViewButton({
  ...
  const [needsLinkConsent, setNeedsLinkConsent] = useState(false);

  const runCheckout = async () => {
    setPending(true);
    try {
      const view = await checkout(datasets, name);
  ...
  const handleClick = () => {
    ...
    setNeedsLinkConsent(!autoLinksCoverThis && computedNewLinkCount > 0);
    setShowDialog(true);
  };
  ...
  return (
    <>
      ...
      {showDialog ? (
        <FgDialog onClose={() => setShowDialog(false)} open={showDialog}>
          <div className="flex flex-col gap-2 my-4">
            ...
            {needsLinkConsent ? (
              <>
                ...
                <Typography className="text-foreground">
                  ...
                  {newLinkCount === 1 ? '' : 's'} and 1 View. If you share the
                  data link(s) with internal collaborators, they will be able to
                  view these data.
                </Typography>
            ...
            <div className="flex gap-4">
              <FgButton
                ...
                onClick={runCheckout}
              >
                {needsLinkConsent ? 'Continue' : 'Create'}
              </FgButton>
              <FgButton
                disabled={pending}
